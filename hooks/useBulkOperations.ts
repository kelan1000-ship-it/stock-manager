...
function cleanAiField(val?: string): string {
  if (!val) return '';
  const v = val.trim();
  return (v.toUpperCase() === 'NA' || v.toUpperCase() === 'N/A' || v.toLowerCase() === 'unknown') ? '' : v;
}

type AiProduct = { barcode?: string; productCode?: string; name?: string; packSize?: string; price?: string; imageUrl?: string };

function mapAiProductToBulkItem(p: AiProduct, fileIndex: number, pIdx: number, masterInventory: MasterProduct[]): BulkItem {
  const barcode = cleanAiField(p.barcode);
  const pip = cleanAiField(p.productCode);
  const name = cleanAiField(p.name);
  const packSize = cleanAiField(p.packSize);
  ...
  if (match) {
    item.originalData = { name, barcode, productCode: pip, packSize, priceStr, costPriceStr: '' };
  }
...
export function useBulkOperations(
      ...
      const results = await Promise.all(
        fileArray.map(async (file, fileIndex) => {
          try {
            ...
            return products.map((p, pIdx) => mapAiProductToBulkItem(p, fileIndex, pIdx, masterInventory));
          } catch (err) {
  ...
  }, [setIsAILoading, setBulkItems, masterInventory]);
