           ...
           <div className={`grid gap-3 w-full ${mainView === 'inventory' ? 'grid-cols-2 md:grid-cols-4 lg:grid-cols-9' : 'grid-cols-1'}`}>
              ...
              {mainView === 'inventory' && (
                <>
                  ...
                  <FilterButton label="Stock Check" value="stock-check" icon={ListChecks} count={branchData[currentBranch]?.filter(p => p.needsStockCheck).length} activeValue={logic.subFilter as InventorySubFilter} onClick={logic.setSubFilter} activeColorClass="bg-amber-600 shadow-amber-900/40" className="w-full justify-center px-2" />
