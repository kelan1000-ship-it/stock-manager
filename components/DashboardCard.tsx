...
export const DashboardCard: React.FC<DashboardCardProps> = ({
  ...
  const colorStyles = {
    ...
    slate: {
      ...
      icon: 'bg-slate-800 border-slate-700 text-slate-500'
    }
  };
  ...
  if (isPlaceholder) {
    return (
      <div 
        onClick={onClick}
        className="h-full min-h-[140px] p-6 rounded-[2rem] border-2 border-dashed border-slate-800 bg-slate-900/30 flex flex-col items-center justify-center gap-3 cursor-pointer hover:bg-slate-900 hover:border-slate-700 transition-all group animate-in fade-in"
      >
        <div className="w-12 h-12 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center text-slate-500 group-hover:scale-110 group-hover:text-emerald-500 group-hover:border-emerald-500/30 transition-all">
          <Plus size={24} />
        </div>
  ...
  return (
    <div 
        ...
        className={`p-6 rounded-[2rem] border flex items-center gap-5 shadow-lg relative overflow-hidden group cursor-pointer transition-all ${
            isActive 
            ? styles.active 
            : `bg-slate-900 border-slate-800 hover:bg-slate-900/80 ${styles.hover}`
        }`}
    >
        {onRemove && (
            <button 
